module.exports = {
  ...
  async down(queryInterface, Sequelize) {
    ...
    const transaction = await queryInterface.sequelize.transaction();
    try {
      await queryInterface.removeColumn('notes', 'branchId', { transaction });

      await queryInterface.removeColumn('leads', 'branchId', { transaction });

      await queryInterface.removeColumn('contacts', 'branchId', {
        transaction,
      });

      await queryInterface.removeColumn('branches', 'branchId', {
        transaction,
      });

      await queryInterface.removeColumn('activities', 'branchId', {
        transaction,
      });

      await queryInterface.removeColumn('users', 'branchId', { transaction });

      await queryInterface.removeColumn('roles', 'globalAccess', {
        transaction,
      });

      await queryInterface.removeColumn('branch', 'name', { transaction });
      ...
      await queryInterface.removeColumn('branches', 'address', { transaction });
